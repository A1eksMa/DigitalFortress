...
Тестирование на клиенте с мобильного устройства Android показало, что при подключении клиента по gRPC тесты не проходят, соединение с Интернетом отсутствует (сайты в браузере не открываются). Соединение через веб-сокет устанавливается, тесты подключения на клиенте проходят, хотя с аномальным по длительности пингом (около 3500-4500 ms)., при этом соединение с сайтами через браузер отсутствует.
